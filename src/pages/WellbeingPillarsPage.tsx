...
export default function WellbeingPillarsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const response: any = await fetchWellbeingPillars(1);
        if (mounted && response?.data?.length) {
          setPillars((response.data as Pillar[]).filter((p) => p.is_active !== 0));
        } else if (mounted) {
  ...
  const maxSelected = (selectedPillars as number[]).length >= 3;

  const handleDone = () => {
    if ((selectedPillars as number[]).length !== 3) {
  ...
  return (
    <Layout wide showLogo={false}>
      ...
      <div className="pillars-grid" role="group" aria-label="Well-being pillars">
        {pillars.map((pillar) => {
          const selectionOrder = (selectedPillars as number[]).indexOf(pillar.id);
          ...
          return (
            <button
              ...
              className={`pillar-item ${
                isSelected ? 'pillar-item--selected' : ''
            ...
            >
              <span className="pillar-item__checkbox" aria-hidden="true">
                {isSelected && (
                  <span className="pillar-item__order">
                    {selectionOrder + 1}
                  </span>
                )}
              </span>
      ...
      <StepActions
        ...
        primaryLabel="Done"
        loading={loading}
        primaryDisabled={(selectedPillars as number[]).length !== 3}
        showBackIcon={false}
      />
    </Layout>
  );
